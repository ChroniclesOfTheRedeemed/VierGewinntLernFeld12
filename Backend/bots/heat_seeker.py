...
from bots.util import in_boundaries, get_level_of_move, return_x_highest_of_array, get_my_mark, get_highest_of_threshold
...
def get_heat_array(game_state):
    spielfeld = game_state["game_field"]
    heat_values = []
    mark = get_my_mark("randy", game_state)
    enemy_mark = get_my_mark("not-randy", game_state)
    for column in range(0, 7):
        ...
        if level != -1:
            ...
            heat_values.append(column_heat_value)
    array = get_highest_of_threshold(heat_values, 0.05)
    random_array = get_random_array()
    result_array = []
    for i in range(0, len(array)):
        result_array.append(array[i] + random_array[i] if array[i] >= 0 else 0)
    return result_array
...
def calculate_opponent_heat_value(column, level, opponent_mark, spielfeld):
    column_heat_value = 0
    my_heat_array = non_vertical_heat_values(column, level, opponent_mark, spielfeld)
    my_heat_array.append(vertical_heat_value(column, level, opponent_mark, spielfeld))
    for heat_val in my_heat_array:
        if heat_val >= 4:
            return 0.9
        elif heat_val == 3:
...
def get_hotness_move(game_state):
    result = return_x_highest_of_array(get_heat_array(game_state), 1)
